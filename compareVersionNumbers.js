...
Output: 0
Explanation: The first version number does not have a third 
level revision number, which means its third level revision number is default to "0"
...
var compareVersion = function(version1, version2) {
  const v1 = version1.split("."), 
        v2 = version2.split("."), 
        end = Math.max(v1.length, v2.length);
  for(let i = 0; i < end; i++){
      const num1 = parseInt(v1[i]) || 0,
            num2 = parseInt(v2[i]) || 0;
      if(num1 === num2){
          continue;
      }
